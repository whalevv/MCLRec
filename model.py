...
class SGNN(Module):
    def __init__(self, hidden_size, step1):
...
class GNN(Module):
    def __init__(self, hidden_size, step2):
...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        self.gnn = SGNN(self.hidden_size, step1=opt.step1)
        self.gnn1 = GNN(self.hidden_size, step2=opt.step2)
    ...
    def compute_scores(self, seq_hidden, hidden, star_node, mask, graph_mask):
        ...
        ht1 = hidden[torch.arange(mask.shape[0]).long(), torch.sum(mask, 1) - 1]
        q11 = self.linear_one1(ht).view(ht1.shape[0], 1, ht1.shape[1])
        q21 = self.linear_two1(hidden)
        alpha1 = self.linear_three1(torch.sigmoid(q11+q21))
        a1 = torch.sum(alpha1 * hidden * mask.view(mask.shape[0], -1, 1), 1)
        a1 = self.linear_transform1(torch.cat([a1, ht1], 1))
        b1 = self.embedding.weight[1:]

        a = layer_normalization(a)
        b = layer_normalization(b)

        a1 = layer_normalization(a)
        b1 = layer_normalization(b)

        pos_ratings_user = torch.sum(a * a1, dim=-1)
        tot_ratings_user = torch.matmul(a, torch.transpose(a1, 0, 1))  # [batch_size, num_users]
...
def forward(model, i, data):
    ...
    hidden, star_node, output1 = model(items, A, graph_mask)
    get = lambda i: hidden[i][alias_inputs[i]]
    seq_hidden = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])

    get1 = lambda i: output1[i][alias_inputs[i]]
...
def train_test(model, train_data, test_data):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        targets, a = forward(model, i, train_data)
        scores, infoNCE_loss = a[0], a[1]
        targets = trans_to_cuda(torch.Tensor(targets)).long()

        loss = model.loss_function(scores, targets - 1)  # + 0.01 * infoNCE_loss
        loss1 = loss + 0.05 * infoNCE_loss
        loss1.backward()
    ...
    for i in slices:
        targets, a = forward(model, i, test_data)
        scores, infoNCE_loss = a[0], a[1]
